...
from CloudExtras import model_get, array_put, array_get, segy_get, resample, get_chunk_size, restrict_model_to_receiver_grid, extent_gradient, extent_gradient, image_mute, image_scaling, data_mute
...
idx = np.random.random_integers(num_shots)
#idx = shot[node_idx]
...
if iteration == 1:
    ...
    x = x[1:].reshape(shape_full[0], shape_full[1], order='F')
    x = restrict_model_to_receiver_grid(sx, gx, x, spacing, origin_full, buffer_size=5000)[0]

    # Apply preconditioners to image
    x = image_mute(x, water)    # mute water column
    x = image_scaling(x, model) # depth scaling
...
# Mute direct wave and anything above (i.e. refracted waves etc.)
dobs = data_mute(dobs, rec_coordinates, tn, dt_comp, mute_start=180, mute_all=1200)

# Predicted data
if iteration > 1:
    dpred = forward_born(model, geometry.src_positions, geometry.src.data, geometry.rec_positions, isic=True)[0]
    dpred = data_mute(dpred, rec_coordinates, tn, dt_comp, mute_start=180, mute_all=1200)
...
# Function value
residual_norm = np.sqrt(np.sum(np.abs(dpred.data[:])**2) * dt_comp)
fval = np.array([.5*residual_norm**2], dtype='float32')
...
g, summary1, summary2 = adjoint_born(model, geometry.rec_positions, dpred.data[:], u=u0, is_residual=True, op_forward=opF, tsub_factor=12, isic=True)
t2 = time.time()
print("Save in memory. Time [s]: ", t2 - t1)

# Remove pml, apply preconditioners and extent back to full size
g = g[model.nbpml:-model.nbpml, model.nbpml:-model.nbpml]  # remove padding
g = image_scaling(g, model) # depth scaling
g = image_mute(g, water)    # mute water column
g = extent_gradient(shape_full, origin_full, shape, origin, spacing, g)
g = np.reshape(g, -1, order='F')

# Append function value
g = np.concatenate([fval, g])
...
filename = 'timings/timings_sgd_iteration_' + str(iteration)
array_put(all_timings, bucket, filename)
